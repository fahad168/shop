...
function close_product_delete_modal() {
    document.getElementById('product-delete-modal').style.display = "none"
    document.getElementById('product_overlay').style.display = "none"
}

function open_product_delete_modal(id) {
    document.getElementById('product-delete-modal').style.display = "flex"
    document.getElementById('product_overlay').style.display = "block"
    document.getElementById('modal_text').innerText = 'Are you sure you want to Delete this Product?'
    document.getElementById('product_destroy').href = `/shop/admin/products/${id}`
}

function check_select_ids(checkbox, id) {
    var input = document.getElementById('product_ids')
    var button = document.getElementById('destroy_button')
    var ids_array = JSON.parse(input.value || "[]")
    if(checkbox.checked) {
        ids_array.push(id)
    } else {
        ids_array = ids_array.filter(item => item !== id);
    }
    if (ids_array.length === 0) {
        button.classList.add('hidden')
    } else {
        button.classList.remove('hidden')
        button.innerText = `${ids_array.length} Delete`
    }
    input.value = JSON.stringify(ids_array)
}

function open_multiple_product_modal() {
    document.getElementById('product-delete-modal').style.display = "flex"
    document.getElementById('product_overlay').style.display = "block"
    var input = document.getElementById('product_ids')
    document.getElementById('modal_text').innerText = 'Are you sure you want to Delete these Products?'
    document.getElementById('product_destroy').href = `/shop/admin/products/multiple_destroy?ids=${input.value}`
}
...
function get_variant_info(select) {
    $.ajax({
        url: `/shop/admin/products/variant_info?id=${select.value}`,
        type: 'GET',
        success: function (res) {
            $('#variant_info').html(res?.variant_info)
            if (res?.images_present) {
                $('#variant_slider').html(res?.slider_images)
            }
        }
    })
}

function change_size_and_stock(size, stock) {
...
function toggleDarkMode() {
    ...
    if (htmlElement.classList.contains('dark')) {
        ...
        $('.theme-switch__checkbox')[0].setAttribute('checked', 'true')
        localStorage.setItem('theme', 'light');
        document.getElementById('before_style').remove()
    } else {
        htmlElement.classList.add('dark');
        $('.theme-switch__checkbox')[0].removeAttribute('checked')
        localStorage.setItem('theme', 'dark');
        add_before_selector_background()
    }
}
...
function add_before_selector_background() {
    var style = document.createElement('style');
    style.setAttribute('id', 'before_style')
    style.innerHTML = `
